...
namespace Battle
{
    public class FOWLogic : Singleton<FOWLogic>
    {
        ...
        public override void Init()
        {
            ...
            Messenger.AddListener<ILit>(MessageName.MN_CHARACTOR_BORN, AddCharactor);
        }

        public override void Dispose()
        {
            Messenger.RemoveListener<ILit>(MessageName.MN_CHARACTOR_BORN, AddCharactor);
        ...
        protected void AddCharactor(ILit lit)
        {
            if (!FOWCharactorRevealer.Contains(lit.id))
            {
                if (FOWCharactorRevealer.CheckIsValid(lit))
                {
                    FOWCharactorRevealer revealer = FOWCharactorRevealer.Get();
                    revealer.InitInfo(lit);
                    FOWSystem.AddRevealer(revealer);
                    m_revealers.Add(revealer);
                    Messenger.Broadcast(MessageName.MN_BoreFire_Change);
                }
            }
        }
        ...
        public FOWRender CreateRender(Transform parent)
        {
            ...
            if (prefabs != null)
            {
                ...
                if (mesh != null)
                {
                    ...
                    meshRenderer.sortingOrder = CntSortingOrder.fogMask;
                }
            }
