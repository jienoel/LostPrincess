...
/*	Ray ray;
	...
	public bool CloseBubble(DialogBubble vcharacter)
	{
	...
	public	void ShowBubble(DialogBubble vcharacter)
	{
		...
		foreach (PixelBubble vBubble in vcharacter.vBubble)
		{
			...
			if (vcharacter.vCurrentBubble == null)
			{
				...
				string vTrueMessage = "";
				string cLine = "";
				int vLimit = vBubble.charcterInLine;

				int i = 0;
				//cut each word in a text in 24 characters.
				foreach (char vWord in vBubble.vMessage)
				{
					i++;
					if (i > vLimit)
					{
				...
				vBubbleObject.transform.localRotation = Quaternion.identity;
				vBubbleObject.transform.localPosition =vBubble.posOffSet;
